...
void pqc_mldsa_polyt1_unpack(pqc_mldsa_poly *r, const uint8_t *a)
{
    unsigned i;
    for (i = 0; i < PQC_MLDSA_N / 4; i++) {
        r->coeffs[4 * i + 0] = (((int32_t)a[5 * i + 0] >> 0) |
                                  ((int32_t)a[5 * i + 1] << 8)) & 0x3FF;
        r->coeffs[4 * i + 1] = (((int32_t)a[5 * i + 1] >> 2) |
                                  ((int32_t)a[5 * i + 2] << 6)) & 0x3FF;
        r->coeffs[4 * i + 2] = (((int32_t)a[5 * i + 2] >> 4) |
                                  ((int32_t)a[5 * i + 3] << 4)) & 0x3FF;
        r->coeffs[4 * i + 3] = (((int32_t)a[5 * i + 3] >> 6) |
                                  ((int32_t)a[5 * i + 4] << 2)) & 0x3FF;
    }
}
...
void pqc_mldsa_polyt0_pack(uint8_t *r, const pqc_mldsa_poly *a)
{
    ...
    for (i = 0; i < PQC_MLDSA_N / 8; i++) {
        t[0] = (1 << (PQC_MLDSA_D - 1)) - a->coeffs[8 * i + 0];
        t[1] = (1 << (PQC_MLDSA_D - 1)) - a->coeffs[8 * i + 1];
        t[2] = (1 << (PQC_MLDSA_D - 1)) - a->coeffs[8 * i + 2];
        t[3] = (1 << (PQC_MLDSA_D - 1)) - a->coeffs[8 * i + 3];
        t[4] = (1 << (PQC_MLDSA_D - 1)) - a->coeffs[8 * i + 4];
        t[5] = (1 << (PQC_MLDSA_D - 1)) - a->coeffs[8 * i + 5];
        t[6] = (1 << (PQC_MLDSA_D - 1)) - a->coeffs[8 * i + 6];
        t[7] = (1 << (PQC_MLDSA_D - 1)) - a->coeffs[8 * i + 7];
                           ...
                           int32_t gamma1)
{
    ...
    if (gamma1 == (1 << 17)) {
        ...
        for (i = 0; i < PQC_MLDSA_N / 4; i++) {
            ...
            r[9 * i + 2] = (uint8_t)(t[0] >> 16) | (uint8_t)(t[1] << 2);
            r[9 * i + 3] = (uint8_t)(t[1] >> 6);
            r[9 * i + 4] = (uint8_t)(t[1] >> 14) | (uint8_t)(t[2] << 4);
            r[9 * i + 5] = (uint8_t)(t[2] >> 4);
            r[9 * i + 6] = (uint8_t)(t[2] >> 12) | (uint8_t)(t[3] << 6);
        ...
        for (i = 0; i < PQC_MLDSA_N / 2; i++) {
            ...
            r[5 * i + 2] = (uint8_t)(t[0] >> 16) | (uint8_t)(t[1] << 4);
                             ...
                             int32_t gamma1)
{
    ...
    if (gamma1 == (1 << 17)) {
        ...
        for (i = 0; i < PQC_MLDSA_N / 2; i++) {
            ...
            r->coeffs[2 * i + 0] |= (int32_t)(a[5 * i + 2] & 0x0F) << 16;
                         ...
                         const pqc_mldsa_params_t *params)
{
    ...
    pqc_mldsa_pack_hint(sig, h, params->k, params->omega);
}
                          ...
                          const pqc_mldsa_params_t *params)
{
    ...
    return pqc_mldsa_unpack_hint(h, sig, params->k, params->omega);
}
